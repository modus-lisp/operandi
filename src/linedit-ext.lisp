...
;;; --- Ctrl-C and Ctrl-Z must always reach the process ------------------
;;; linedit enters its raw mode with cfmakeraw, which clears ISIG along with
;;; everything else, so while it waits for input the terminal never sends
;;; SIGINT or SIGTSTP — ^C and ^Z arrive as bytes, and linedit's own C-C
;;; command turns ^C into kill(0, SIGINT). Fine while linedit is healthy.
;;; But if its input loop stops answering, NOTHING typed can reach the
;;; process: that is the "can't even ^Z it" session, caught later with its
;;; tty at -icanon -isig and its main thread parked in select for two days.
;;;
;;; So put ISIG back after linedit sets raw mode. Line editing is unchanged
;;; (canonical mode and echo stay off); ^C and ^Z just become real signals
;;; again, generated by the terminal, which works whether or not linedit is
;;; listening. The REPL already turns a SIGINT at the prompt into a fresh
;;; prompt (the handler-case around read-input).

(defmethod linedit::backend-init :after ((backend linedit::terminal))
  (ignore-errors
   (let ((tio (sb-posix:tcgetattr 0)))
     (setf (sb-posix:termios-lflag tio)
           (logior (sb-posix:termios-lflag tio) sb-posix:isig))
